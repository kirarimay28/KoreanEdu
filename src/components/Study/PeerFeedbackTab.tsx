            ...
            {received.map(f => (
              <div key={f.id} className="card p-3 space-y-1.5">
                <div className="flex items-center gap-2">
                  <NameWithCrown name={f.authorName} className="text-xs font-semibold text-gray-700" showAvatar avatarSize="xs" />
        ...
        {sent.length === 0 ? (
          ...
          <div className="space-y-2">
            {sent.map(f => (
              <div key={f.id} className="card p-3 space-y-1.5">
                <div className="flex items-center gap-2">
                  <NameWithCrown name={f.targetName} className="text-xs font-semibold text-gray-700" showAvatar avatarSize="xs" />
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${CAT_COLOR[f.category as PeerFeedbackCategory]}`}>{f.category}</span>
                  <button
                ...
                return (
                  <div key={f.id} className={`card p-3 space-y-1.5 ${isMe ? 'border-primary-100' : ''}`}>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <NameWithCrown name={f.authorName} className={`text-xs font-bold ${f.authorId === currentUser.id ? 'text-primary-600' : 'text-gray-700'}`} showAvatar avatarSize="xs" />
                      <ArrowRight className="w-3 h-3 text-gray-300 flex-shrink-0" />
                      <NameWithCrown name={f.targetName} className={`text-xs font-bold ${f.targetId === currentUser.id ? 'text-primary-600' : 'text-gray-700'}`} showAvatar avatarSize="xs" />
